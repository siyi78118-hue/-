...
assert.match(readFileSync(executionDbPath, 'utf8'), /version\s*=\s*11[\s\S]*MIGRATION_2_3[\s\S]*MIGRATION_3_4[\s\S]*MIGRATION_4_5[\s\S]*MIGRATION_5_6[\s\S]*MIGRATION_6_7[\s\S]*MIGRATION_7_8[\s\S]*MIGRATION_8_9[\s\S]*MIGRATION_9_10[\s\S]*MIGRATION_10_11/, 'Room must migrate existing installs through the conversation-authority schema without destructive reset');
