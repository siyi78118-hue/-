...
assert.match(swScript, /const CACHE_NAME = 'rpchat-v18';/);
assert.match(script, /const MEMORY_DB_VERSION = 2;/);
assert.match(swScript, /const MEMORY_DB_VERSION = 2;/);
assert.match(script, /const APP_BUILD_VERSION = '2026-07-09\.6';/);
assert.match(script, /const EXPECTED_CLOUD_TIMER_VERSION = '2026-07-09\.3';/);
...
assert.match(cloudTimerWorker, /const CLOUD_TIMER_WORKER_VERSION = '2026-07-09\.3';/);
assert.match(cloudTimerWorker, /version: CLOUD_TIMER_WORKER_VERSION/);
assert.equal(packageJson.scripts['cloud:deploy'], 'wrangler deploy');
assert.equal(packageJson.scripts['cloud:health'], 'node scripts/check-cloud-timer.mjs');
assert.match(cloudTimerHealthScript, /EXPECTED_VERSION = '2026-07-09\.3'/);
