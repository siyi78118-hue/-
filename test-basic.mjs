...
assert.match(swScript, /const CACHE_NAME = 'rpchat-v41';/);
assert.match(script, /const MEMORY_DB_VERSION = 2;/);
assert.match(swScript, /const MEMORY_DB_VERSION = 2;/);
assert.match(script, /const APP_BUILD_VERSION = '2026-07-09\.27';/);
...
assert.match(script, /withTimeout\(cancelCloudProactive\(deletedId, 'all'\), 8000, '取消云闹钟超时'\)/);
