...
assert.match(swScript, /const CACHE_NAME = 'rpchat-v51';/);
assert.match(script, /const MEMORY_DB_VERSION = 2;/);
assert.match(swScript, /const MEMORY_DB_VERSION = 2;/);
assert.match(script, /const APP_BUILD_VERSION = '2026-07-09\.37';/);
...
assert.match(html, /addMemoryItem\('events'\)/);
assert.match(html, /addMemoryItem\('summaries'\)/);
...
assert.match(script, /let memoryEditorState = \{ storeName: 'profiles', id: '' \};/);
...
assert.match(cloudTimerWorker, /const CLOUD_TIMER_WORKER_VERSION = '2026-07-09\.6';/);
...
assert.match(cloudTimerHealthScript, /EXPECTED_VERSION = '2026-07-09\.6'/);
