...
test('formal signing uses the explicit Android release version instead of the workflow run number', () => {
  assert.match(androidBuild, /AL_VERSION_CODE"\) \?: "107"/);
  assert.match(androidBuild, /AL_VERSION_NAME"\) \?: "1\.0\.107"/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_CODE:\s*107/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_NAME:\s*1\.0\.107/);
...
test('the signed branch build publishes an OTA manifest that names the real release asset', () => {
  assert.equal(updateManifest.latestBuild, 107);
