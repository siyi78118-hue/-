...
test('formal signing uses the explicit Android release version instead of the workflow run number', () => {
  assert.match(androidBuild, /AL_VERSION_CODE"\) \?: "91"/);
  assert.match(androidBuild, /AL_VERSION_NAME"\) \?: "1\.0\.91"/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_CODE:\s*91/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_NAME:\s*1\.0\.91/);
