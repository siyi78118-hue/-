...
test('formal signing uses the explicit Android release version instead of the workflow run number', () => {
  assert.match(androidBuild, /AL_VERSION_CODE"\) \?: "116"/);
  assert.match(androidBuild, /AL_VERSION_NAME"\) \?: "1\.0\.116"/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_CODE:\s*116/);
  assert.match(androidWorkflow, /AL_RELEASE_VERSION_NAME:\s*1\.0\.116/);
...
test('the signed branch build publishes an OTA manifest that names the real release asset', () => {
  assert.equal(updateManifest.latestBuild, 116);
  assert.equal(updateManifest.version, '1.0.116');
  assert.equal(
    updateManifest.releaseUrl,
    'https://github.com/siyi78118-hue/-/releases/download/android-v116/app-release.apk'
  );
  assert.match(
    androidWorkflow,
