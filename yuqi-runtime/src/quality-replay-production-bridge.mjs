...
const RELEASE_REASONING_EFFORTS = new Set(['none', 'low', 'medium', 'high', 'xhigh', 'max']);
...
function parseReleaseModelProfile(modelProfile, label = 'release model profile') {
  ...
  for (const key of RELEASE_PROFILE_KEYS) {
    ...
    if (typeof value !== 'string' || !value || value !== value.trim()) throw new Error(`${label} ${key} conflict`);
