...
test('loads a checksummed immutable seed version', () => withRegistry(registry => {
  ...
  assert.equal(current.version, '1.4.0');
...
test('promotes an older current seed to the newer packaged preset version', () => {
  ...
  try {
    ...
    assert.equal(registry.current().version, '1.4.0');
  } finally {
    store.close();
    rmSync(dir, { recursive: true, force: true });
  }
...
test('publishes an annotation as a child version and can roll back immutably', () => withRegistry((registry, store) => {
  ...
  assert.equal(published.version, '1.4.1');
  assert.equal(published.parentVersion, '1.4.0');
  ...
  const rollback = registry.rollback('1.4.0');
  assert.equal(rollback.version, '1.4.2');
  assert.equal(rollback.parentVersion, '1.4.1');
  assert.equal(rollback.rollbackOf, '1.4.0');
