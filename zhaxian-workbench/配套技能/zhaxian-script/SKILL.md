...
### 情况 A0（推荐 · 本地桥接自动模式）：用户说「继续 / 下一步 / 处理请求 / 跑一下」
说明用户开了桥接服务器（`python3 bridge.py`），在网页点了「✨ 发给 AI」，请求已被写进文件。我要：
1. **读取** `谪仙工程/请求.txt`——它的内容就是网页生成的整段指令（自带 `_step` + 【本步预设】+ 当前工程数据），等同于情况 A 里用户原本要粘贴给我的东西。
2. 按里面的 `_step` 与【本步预设】严格执行，生成结果。
3. **用 Write 把【纯 JSON】写入** `谪仙工程/ai-out.json`（外层带 `_step`，无围栏无解释，整文件能被 `JSON.parse`）。
4. **处理完把 `谪仙工程/请求.txt` 清空**（写入空字符串），避免重复处理。
5. 只回一句：`第X步已处理，网页会自动填回 ✅`。网页每 3 秒自动轮询，无需用户再点【读取】。

> 若 `请求.txt` 为空或不存在，就告诉用户：「没收到新请求，请先在网页点『✨ 发给 AI』」。
> 想全自动、连「继续」都不说：用户可对我用 `/loop 处理请求`，我会定时查 `请求.txt`，有新内容就处理。
