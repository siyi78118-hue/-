...
if (config.cloudRelay.enabled && config.cloudRelay.url) {
  cloud = await requestCloudJson(`${config.cloudRelay.url.replace(/\/+$/, '')}/bridge/health`)
    .then(body => ({ status: 200, body }))
    .catch(error => ({ status: 0, body: { error: error.message } }));
}
const cloudRelayReady = cloud.status === 200 && cloud.body.ok === true && cloud.body.storage === 'ciphertext-only';

const result = {
  ok: local.status === 200 && local.body.ok === true && sessions.length === 3 && snapshots.length > 0
    && pairingReady && existsSync(apkPath) && (!config.cloudRelay.enabled || cloudRelayReady),
