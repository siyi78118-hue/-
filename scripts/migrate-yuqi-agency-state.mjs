...
function v14InvariantSummary(store) {
  store.assertAgencyV10Invariants();
  return store.releaseAuthorityV14InvariantSummary();
}
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  ...
  const store = apply
    ? YuqiStore.openForMigration(workingDatabase, {
        ...
        expectedPostMigrationInvariantChecksum: expected.v14InvariantSummary.checksum
      })
  ...
  try {
    ...
    report.v14InvariantSummary = v14InvariantSummary(store);
  } finally {
    store.close();
  }
  ...
  if (expectedPath) {
    ...
    if (expected.decisionChecksum !== report.decisionChecksum
      ...
      || canonicalJson(expected.v14InvariantSummary) !== canonicalJson(report.v14InvariantSummary)) {
